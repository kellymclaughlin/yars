...
-spec get_next_pid(term()) -> pid() | {'error', gcp_error_reason()}.
...
get_next_pid(Name) ->
    ...
    case ets:member(pg2_table, {group, Name}) of
        false ->
            {error, {no_such_group, Name}};
        true ->
            {[{_, NextProcessIndex} | _], _BadNodes} = global:trans({{?MODULE, Name}, self()},
                         fun() ->
                                 gen_server:multi_call(?MODULE,
                                                       next_process)
                         end),
            case NextProcessIndex of 
                unavailable ->
                    {error, {no_process, Name}};
                _ ->
                    case get_members(Name) of
...
-record(state, {process_count, next_process}).
...
init([]) ->
    ...
    {ok, #state{process_count=0, next_process=1}}.
...
handle_call({delete, Name}, _From, S) ->
    delete_group(Name),
    {reply, ok, S};
handle_call(next_process, _From, S) ->
    case S#state.process_count > 0 of
        true ->
            case S#state.next_process == S#state.process_count of
                true ->
                    {reply, S#state.next_process, S#state{next_process=1}};
                false ->
                    {reply, S#state.next_process, S#state{next_process=S#state.next_process+1}}
            end;
        false ->
            {reply, unavailable, S}
    end;
